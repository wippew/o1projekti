...
package object util extends iofuncs {
  ...
  implicit class ConvenientCollection[Element, Collection[Element] <: Traversable[Element]](private val self: Collection[Element]) extends AnyVal {
    ...
    /** Constructs and returns a new map from the collection by applying an ID-generating function
      * to each element of the collection and using the collection’s elements as values. This is
    ...
    /** Constructs and returns a new map from the collection by applying a value-generating function
      * to each element of the collection and using the collection’s elements as keys. This is
      * equivalent to calling `mapify(identity)(formValue)`.
      * @param formID    a function called on each element of the collection to obtain the keys */
    def mapTo[Value](formValue: Element => Value): Map[Element, Value] =
      self.mapify(identity)(formValue)


    /** Constructs and returns a new map with the collection’s elements as keys and each
      * element’s occurrence counts as the corresponding values. This is equivalent to calling
      * `groupBy(identity).mapValues( _.size )`. */
    def occurrences: Map[Element, Int] = self.groupBy(identity).mapValues( _.size )
    ...
    def log(format: Traversable[Element] => String = _.toString ): Collection[Element] = {
      println(format(self))
      self
    }
  ...
  implicit class ConvenientOption[Content](private val self: Option[Content]) extends AnyVal {
    ...
    /** Prints out a report of the `Option`’s contents (if any). Returns the unmodified `Option`.
      ...
      * @param descrineNone     a description of an empty `Option`; defaults to `"None"` */
