...
case class GridPos(val x: Int, val y: Int) {
  ...
  /** Determines if the given `GridPos` is west or east of this one. Returns `West` if this
    * `GridPos`’s x coordinate is less than `another`’s and `East` if the opposite is true.
  ...
  /** Determines if the given `GridPos` is north or south of this one. Returns `North` if this
    * `GridPos`’s y coordinate is less than `another`’s and `South` if the opposite is true.
