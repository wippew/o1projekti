...
abstract class Grid[Element: ClassTag](val width: Int, val height: Int) {
  ...
  private val contents = {
    val elems = Try(this.initialElements) match {
      case Success(elems) => elems
      case Failure(npe: NullPointerException) => throw new RuntimeException("Grid initialization failed with a NullPointerException.\nPossible cause: trying to access an element’s (still nonexistent) neighbors or other parts of the unready grid while initializing.", npe)
      case Failure(other) => throw other
    }
    require(elems.size == this.size, s"The number of elements returned by initialElements (${elems.size}) did not equal width times height ($size).")
